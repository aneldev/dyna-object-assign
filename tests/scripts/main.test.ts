...
describe('dyna object assign with undefined objects', () => {
  it('should not assign undefined values', () => {
    const objA: any = {name: "John", age: 32, address: undefined, city: null};
    const objB: any = undefined;
    const objC: any = null;
    const objD: any = {age: undefined, address: "Lungerstrasse 12"};
    const obj: any = dynaObjectAssign(objA, objB, objC, objD);
    expect(obj.name).toBe("John");
    expect(obj.age).toBe(32);
    expect(obj.address).toBe("Lungerstrasse 12");
    expect(obj.city).toBe(null);
